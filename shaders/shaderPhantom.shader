...
vec4 colorizeColor(vec4 inColor, vec4 targetColor)
{
    vec4 grayscale = rgbToHsl(inColor);
    grayscale.g = 0.0;
    return hslToRgb(grayscale) * targetColor;
}

void main()
{
    gl_FragColor = colorizeColor(v_vColour * texture2D(gm_BaseTexture, v_vTexcoord), phantomColor);
}
